...
try:
    ...
    print("ERROR: Could not import components from 'gemini_analyzer.py'.")
    print("Ensure 'gemini_analyzer.py' is in the same directory or accessible in the Python path.")
    sys.exit(1)
        ...
        if final_results: #
            if final_results.get("error"): # Check final error status #
                ...
                if final_results.get("steps"):
                 ...
                 # Use a distinct filename for agent analysis
...
if __name__ == "__main__": #
    try:
        ...
        asyncio.run(run_main_analysis_terminal()) #
         ...
         if 'gemini_analyzer' in str(e): # Check specifically for the reused module #
             print("❌ ERROR: Cannot import required components from `gemini_analyzer.py`.") #
             print("➡️ Please ensure `gemini_analyzer.py` exists in the same directory or Python path.") #
         else: #
